...
const getAdaptiveQualityTier = (
  ...
  const compactViewport = isCompactViewport(viewport);
  const shortViewport = isShortViewport(viewport);
  ...
  // Touch-capable laptops should stay on the desktop-quality path when their
  // viewport is full-size; input type is not a reliable proxy for layout.
  capabilityScore += viewportArea >= 900_000 ? 1 : 0;
  capabilityScore += viewport.dpr <= 2.2 ? 1 : 0;
  capabilityScore += shortViewport ? -1 : 0;
  ...
  return capabilityScore >= 0 ? "high" : "low";
};
...
export function useSectionActivity<T extends HTMLElement = HTMLElement>(
  ...
  const qualityTier = useMemo(
    () => getAdaptiveQualityTier(prefersReducedMotion, viewportMetrics),
    [prefersReducedMotion, viewportMetrics],
  );
