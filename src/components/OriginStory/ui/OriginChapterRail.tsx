...
const OriginChapterRail: React.FC<OriginChapterRailProps> = ({ beats, activeIndex, disabled = false, onSelect }) => {
  return (
    <aside className={`pointer-events-auto absolute right-4 top-1/2 z-40 -translate-y-1/2 ${disabled ? "opacity-40" : ""}`}>
      <ol className={`origin-chapter-rail ${disabled ? "is-disabled" : ""}`}>
