...
const createBlogCoverSvg = ({
  ...
  const [deep, bright, line] = palette;
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 900" fill="none">
      <defs>
        <linearGradient id="bg" x1="120" y1="80" x2="1080" y2="760" gradientUnits="userSpaceOnUse">
          <stop stop-color="${deep}" />
          <stop offset="0.54" stop-color="${bright}" />
          <stop offset="1" stop-color="#F8FAFC" stop-opacity="0.96" />
        </linearGradient>
        <radialGradient id="glow" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(860 220) rotate(115) scale(460 540)">
          <stop stop-color="#FFFFFF" stop-opacity="0.72" />
          <stop offset="1" stop-color="#FFFFFF" stop-opacity="0" />
        </radialGradient>
      </defs>
      <rect width="1200" height="900" rx="80" fill="url(#bg)" />
      <rect x="78" y="78" width="1044" height="744" rx="58" fill="rgba(255,255,255,0.12)" stroke="${line}" stroke-width="2" />
      <ellipse cx="860" cy="220" rx="340" ry="280" fill="url(#glow)" />
      <path d="M132 668C268 570 376 532 510 542C640 552 744 628 864 624C967 620 1041 571 1080 532" stroke="${line}" stroke-opacity="0.56" stroke-width="3" />
      <path d="M132 722C281 636 386 614 516 624C647 634 764 702 885 698C971 695 1037 664 1080 632" stroke="${line}" stroke-opacity="0.28" stroke-width="2" />
      <text x="132" y="164" fill="rgba(15,23,42,0.72)" font-family="Manrope, Arial, sans-serif" font-size="28" letter-spacing="6">${kicker}</text>
      <text x="132" y="316" fill="#0F172A" font-family="Manrope, Arial, sans-serif" font-size="92" font-weight="600">${title}</text>
      <text x="132" y="382" fill="rgba(15,23,42,0.58)" font-family="Manrope, Arial, sans-serif" font-size="30">Placeholder article cover for the research collage foundation.</text>
    </svg>
  `;
...
const createPlaceholderPost = (input: Omit<BlogPostEntry, "coverImage"> & { coverPalette: [string, string, string] }): BlogPostEntry =>
  defineBlogPost({
    ...input,
    coverImage: {
      src: createBlogCoverSvg({
        title: input.title,
        kicker: input.tag,
        palette: input.coverPalette,
      }),
      alt: `${input.title} placeholder cover artwork`,
    },
  });
